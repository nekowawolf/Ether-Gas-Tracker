...
let AbstractTezosRpc = class AbstractTezosRpc {
    async sendGet({ path, queryParams }) {
        if (queryParams && Object.keys(queryParams).length > 0) {
            return this.get({ path: util_1.Utils.addQueryParams(path, util_1.Utils.camelToSnakeCase, queryParams) });
        }
    ...
    async sendPost({ path, body, queryParams, }) {
        ...
        if (queryParams && Object.keys(queryParams).length > 0) {
            post.path = util_1.Utils.addQueryParams(path, util_1.Utils.camelToSnakeCase, queryParams);
        }
        if (body) {
